...
const reposeCurrentSchemaVersion = 11
...
func openInventoryStore(ctx context.Context, filename string, create bool) (*inventoryStore, error) {
	...
	if version == 8 {
		if _, err := base.db.ExecContext(ctx, reposePromptSchemaSQL); err != nil {
			return nil, fmt.Errorf("upgrade Repose prompt storage: %w", err)
		}
		version = 9
	}
	if version == 9 {
		if _, err := base.db.ExecContext(ctx, reposeFindingAttributionSchemaSQL); err != nil {
			return nil, fmt.Errorf("upgrade Repose finding attribution storage: %w", err)
		}
		version = 10
	}
	if version == 10 {
		if _, err := base.db.ExecContext(ctx, reposeFixSchemaSQL); err != nil {
			return nil, fmt.Errorf("upgrade Repose fix queue storage: %w", err)
		}
		version = 11
	}
