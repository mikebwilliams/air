...
.toolbar.verification-enabled{grid-template-columns:minmax(220px,1fr) repeat(4,minmax(125px,auto))}.verification-confirmed{color:var(--error);background:var(--error-bg)}.verification-false_positive{color:var(--dismissed);background:var(--raised)}.verification-uncertain,.verification-unchecked{color:var(--muted);background:var(--raised)}
@media(max-width:800px){.page{padding:14px}.masthead{display:block}.summary{justify-content:flex-start;margin-top:12px}.toolbar,.toolbar.verification-enabled{grid-template-columns:1fr 1fr}.toolbar .search{grid-column:1/-1}.workspace{display:block;height:auto;min-height:0}.finding-list{height:42vh;border-right:0;border-bottom:1px solid var(--border)}.detail{min-height:55vh;padding:18px}}
...
var state={query:'',status:report.initial_status||'open',severity:'all',verification:report.verification||'all',sort:'id',selected:null};
...
function searchable(f){return [f.id,f.severity,f.disposition,f.title,f.description,f.file,f.line,f.symbol,(f.tags||[]).join(' '),f.author,f.commit_date,f.introduced_sha,f.resolved_sha,f.dismiss_reason,f.observed_sha,f.observed_at,f.scan_id,f.task_id,f.attempt_id,verdictLabel(verdict(f)),JSON.stringify(f.review||{}),JSON.stringify(f.verifications||[])].join(' ').toLowerCase()}
...
function renderDetail(f){detail.replaceChildren();if(!f){detail.appendChild(node('div','empty','No findings match the current filters.'));return}detail.appendChild(node('h2','',f.title));var badges=node('div','badges');addBadge(badges,'#'+f.id,'disposition disposition-'+f.disposition);addBadge(badges,f.severity,'severity-'+f.severity);addBadge(badges,f.disposition,'disposition disposition-'+f.disposition);(f.tags||[]).forEach(function(tag){addBadge(badges,tag,'disposition')});detail.appendChild(badges);detail.appendChild(node('h3','','Description'));detail.appendChild(node('div','description',f.description));var metadata=node('dl','metadata');addMeta(metadata,'Location',location(f),true);addMeta(metadata,'Tags',(f.tags||[]).join(', '),true);if(report.snapshot){addMeta(metadata,'Observed snapshot',f.observed_sha,true);addMeta(metadata,'Observed',f.observed_at?new Date(f.observed_at).toLocaleString():'');addMeta(metadata,'Observation age',age(f));addMeta(metadata,'Scan',f.scan_id,true);addMeta(metadata,'Assignment',f.task_id,true);addMeta(metadata,'Attempt',f.attempt_id)}else{addMeta(metadata,'Introduced',shortSHA(f.introduced_sha),true);addMeta(metadata,'Commit age',age(f));addMeta(metadata,'Author',f.author);addMeta(metadata,'Commit date',f.commit_date?new Date(f.commit_date).toLocaleString():'')}addMeta(metadata,'Resolved',shortSHA(f.resolved_sha),true);addMeta(metadata,'Dismissed',f.dismissed_at?new Date(f.dismissed_at).toLocaleString():'');addMeta(metadata,'Dismissal reason',f.dismiss_reason);if(f.review){addMeta(metadata,'Review','#'+f.review.number+'  '+(f.review.harness&&f.review.harness!='codex'?f.review.harness+':':'')+f.review.model+(f.review.reasoning_effort?'/'+f.review.reasoning_effort:''));addMeta(metadata,'Reviewed',new Date(f.review.reviewed_at).toLocaleString())}detail.appendChild(metadata);if(report.snapshot)detail.appendChild(renderVerifications(f));detail.appendChild(renderDiff(f));detail.appendChild(renderHistory(f))}
...
search.addEventListener('input',function(){state.query=search.value;render()});statusSelect.addEventListener('change',function(){state.status=statusSelect.value;render()});severitySelect.addEventListener('change',function(){state.severity=severitySelect.value;render()});sortSelect.addEventListener('change',function(){state.sort=sortSelect.value;render()});verificationSelect.addEventListener('change',function(){state.verification=verificationSelect.value;render()});
...
if(report.snapshot){verificationSelect.hidden=false;verificationSelect.value=state.verification;document.querySelector('.toolbar').classList.add('verification-enabled');sortSelect.querySelector('[value=author]').remove();statusSelect.querySelector('[value=resolved]').remove()}
statusSelect.value=state.status;
document.title=report.title||'Findings Report';document.getElementById('report-title').textContent=report.title||'Findings Report';document.getElementById('report-meta').textContent=report.repository+(report.scan_id?' · scan '+shortSHA(report.scan_id):'')+' · generated '+new Date(report.generated_at).toLocaleString();renderSummary();render();
})();
</script>
</body>
</html>
