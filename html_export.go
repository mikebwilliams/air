...
type htmlExportFinding struct {
	...
	BlamedSHA     string              `json:"blamed_sha,omitempty"`
	BlamedAt      string              `json:"blamed_at,omitempty"`
...
func makeHTMLExportFinding(
	...
	if finding.Attribution != nil {
		if finding.Attribution.Status == attributionStatusAttributed {
			exported.Author = finding.Attribution.Author
			exported.AuthorEmail = finding.Attribution.AuthorEmail
			exported.BlamedSHA = finding.Attribution.CommitSHA
			if finding.Attribution.AuthoredAt != nil {
				exported.BlamedAt = finding.Attribution.AuthoredAt.Format(time.RFC3339)
			}
		} else {
			exported.BlameError = finding.Attribution.Error
		}
	}
...
var state={query:'',status:report.initial_status||'open',severity:'all',verification:report.verification||'all',tags:[],untagged:false,sort:'id',selected:null,linked:false};
...
function findingDate(f){return report.snapshot?f.blamed_at:f.commit_date}
...
function matchesTags(f){var tags=findingTags(f);return(!state.untagged||tags.length===0)&&state.tags.every(function(tag){return tags.indexOf(tag)!==-1})}
function matchesBaseFilters(f){return(state.status==='all'||f.disposition===state.status)&&(state.severity==='all'||f.severity===state.severity)&&(state.verification==='all'||verdict(f)===state.verification)&&matchesSearch(f)}
function allTagCounts(){var counts={};report.findings.forEach(function(f){findingTags(f).forEach(function(tag){counts[tag]=(counts[tag]||0)+1})});return counts}
function remainingTagCounts(){var result={tags:{},untagged:0};Object.keys(tagControls).forEach(function(tag){result.tags[tag]=0});report.findings.forEach(function(f){if(!matchesBaseFilters(f)||!matchesTags(f))return;var tags=findingTags(f);if(!tags.length)result.untagged++;tags.forEach(function(tag){result.tags[tag]++})});return result}
function renderTagSummary(){tagSummary.textContent=state.untagged?'Untagged':state.tags.length?state.tags.length===1?'Tag: '+state.tags[0]:state.tags.length+' tags (AND)':'All tags'}
function updateTagControl(control,count,selected,title){control.checkbox.checked=selected;control.checkbox.disabled=!selected&&count===0;control.label.classList.toggle('unavailable',control.checkbox.disabled);control.count.textContent=count;control.count.title=title}
function refreshTagOptions(){var counts=remainingTagCounts();updateTagControl(untaggedControl,counts.untagged,state.untagged,state.untagged?'Findings matching the current filters':'Findings with no tags');Object.keys(tagControls).forEach(function(tag){var selected=state.tags.indexOf(tag)!==-1;updateTagControl(tagControls[tag],counts.tags[tag],selected,selected?'Findings matching the current filters':'Findings after also requiring '+tag)});document.querySelector('.tag-clear').disabled=!state.untagged&&!state.tags.length;renderTagSummary()}
function renderTagOptions(){var totals=allTagCounts(),tags=Object.keys(totals).sort();tagFilter.hidden=!report.findings.length;toolbar.classList.toggle('tags-enabled',report.findings.length>0);tagOptions.replaceChildren();tagControls={};var untaggedLabel=node('label','tag-option'),untaggedCheckbox=node('input'),untaggedCount=node('span','tag-option-count');untaggedCheckbox.type='checkbox';untaggedCheckbox.addEventListener('change',function(){state.untagged=untaggedCheckbox.checked;refreshTagOptions();render()});untaggedLabel.append(untaggedCheckbox,node('span','','Untagged'),untaggedCount);tagOptions.appendChild(untaggedLabel);untaggedControl={label:untaggedLabel,checkbox:untaggedCheckbox,count:untaggedCount};var clear=node('button','tag-clear','Clear tag filters');clear.type='button';tags.forEach(function(tag){var label=node('label','tag-option');var checkbox=node('input');var count=node('span','tag-option-count');checkbox.type='checkbox';checkbox.addEventListener('change',function(){if(checkbox.checked)state.tags.push(tag);else state.tags=state.tags.filter(function(selected){return selected!==tag});state.tags.sort();refreshTagOptions();render()});label.append(checkbox,node('span','',tag),count);tagOptions.appendChild(label);tagControls[tag]={label:label,checkbox:checkbox,count:count}});clear.addEventListener('click',function(){state.untagged=false;state.tags=[];refreshTagOptions();render()});tagOptions.appendChild(clear);refreshTagOptions()}
...
function replaceFindingHash(id){history.replaceState(null,'','#'+id)}
function selectFinding(id,link){state.selected=id;if(link){state.linked=true;replaceFindingHash(id)}render()}
function hashFindingID(){var match=window.location.hash.match(/^#([1-9][0-9]*)$/);if(!match)return null;var id=Number(match[1]);return Number.isSafeInteger(id)?id:null}
function revealHashFinding(){var id=hashFindingID();if(id===null){state.linked=false;return false}var finding=report.findings.find(function(candidate){return candidate.id===id});if(!finding){state.linked=false;return false}if(state.status!=='all'&&finding.disposition!==state.status){state.status='all';statusSelect.value='all'}if(state.severity!=='all'&&finding.severity!==state.severity){state.severity='all';severitySelect.value='all'}if(state.verification!=='all'&&verdict(finding)!==state.verification){state.verification='all';verificationSelect.value='all'}if(!matchesSearch(finding)){state.query='';search.value=''}if(!matchesTags(finding)){state.tags=[];state.untagged=false}state.selected=id;state.linked=true;refreshTagOptions();render();return true}
function addTagBadge(parent,tag){var badge=node('button','badge disposition tag-badge',tag);badge.type='button';badge.title='Require tag '+tag;badge.addEventListener('click',function(){if(state.tags.indexOf(tag)===-1){state.tags.push(tag);state.tags.sort();refreshTagOptions();render()}});parent.appendChild(badge)}
function renderDetail(f){detail.replaceChildren();if(!f){detail.appendChild(node('div','empty','No findings match the current filters.'));return}detail.appendChild(node('h2','',f.title));var badges=node('div','badges');var link=node('a','badge disposition disposition-'+f.disposition+' finding-link','#'+f.id);link.href='#'+f.id;link.title='Link to finding #'+f.id;link.addEventListener('click',function(event){event.preventDefault();selectFinding(f.id,true)});badges.appendChild(link);addBadge(badges,f.severity,'severity-'+f.severity);addBadge(badges,f.disposition,'disposition disposition-'+f.disposition);findingTags(f).forEach(function(tag){addTagBadge(badges,tag)});detail.appendChild(badges);detail.appendChild(node('h3','','Description'));detail.appendChild(node('div','description',f.description));var metadata=node('dl','metadata');addMeta(metadata,'Location',location(f),true);addMeta(metadata,'Tags',findingTags(f).join(', '),true);if(report.snapshot){addMeta(metadata,'Observed snapshot',f.observed_sha,true);addMeta(metadata,'Observed',f.observed_at?new Date(f.observed_at).toLocaleString():'');addMeta(metadata,'Line age',age(f));addMeta(metadata,'Author',f.author);addMeta(metadata,'Blamed commit',shortSHA(f.blamed_sha),true);addMeta(metadata,'Blamed commit date',f.blamed_at?new Date(f.blamed_at).toLocaleString():'');addMeta(metadata,'Attribution error',f.blame_error);addMeta(metadata,'Scan',f.scan_id,true);addMeta(metadata,'Assignment',f.task_id,true);addMeta(metadata,'Attempt',f.attempt_id)}else{addMeta(metadata,'Introduced',shortSHA(f.introduced_sha),true);addMeta(metadata,'Commit age',age(f));addMeta(metadata,'Author',f.author);addMeta(metadata,'Commit date',f.commit_date?new Date(f.commit_date).toLocaleString():'')}addMeta(metadata,'Resolved',shortSHA(f.resolved_sha),true);addMeta(metadata,'Dismissed',f.dismissed_at?new Date(f.dismissed_at).toLocaleString():'');addMeta(metadata,'Dismissal reason',f.dismiss_reason);if(f.review){addMeta(metadata,'Review','#'+f.review.number+'  '+(f.review.harness&&f.review.harness!='codex'?f.review.harness+':':'')+f.review.model+(f.review.reasoning_effort?'/'+f.review.reasoning_effort:''));addMeta(metadata,'Reviewed',new Date(f.review.reviewed_at).toLocaleString())}detail.appendChild(metadata);if(report.snapshot)detail.appendChild(renderVerifications(f));detail.appendChild(renderDiff(f));detail.appendChild(renderHistory(f))}
function render(){var items=visibleFindings(),previous=state.selected;if(!items.some(function(f){return f.id===state.selected}))state.selected=items.length?items[0].id:null;if(state.linked&&state.selected!==previous&&state.selected!==null)replaceFindingHash(state.selected);renderList(items);renderDetail(items.find(function(f){return f.id===state.selected})||null)}
search.addEventListener('input',function(){state.query=search.value;refreshTagOptions();render()});search.addEventListener('keydown',function(event){if(event.key==='Escape'){search.value='';state.query='';search.blur();refreshTagOptions();render()}});statusSelect.addEventListener('change',function(){state.status=statusSelect.value;refreshTagOptions();render()});severitySelect.addEventListener('change',function(){state.severity=severitySelect.value;refreshTagOptions();render()});sortSelect.addEventListener('change',function(){state.sort=sortSelect.value;render()});verificationSelect.addEventListener('change',function(){state.verification=verificationSelect.value;refreshTagOptions();render()});
document.addEventListener('keydown',function(event){var form=/^(INPUT|SELECT|TEXTAREA)$/.test(event.target.tagName);if(event.key==='/'&&!form){event.preventDefault();search.focus();return}if(form)return;var items=visibleFindings();var index=items.findIndex(function(f){return f.id===state.selected});if(event.key==='ArrowDown'||event.key==='j'){event.preventDefault();if(items.length)selectFinding(items[Math.min(items.length-1,index+1)].id,true)}else if(event.key==='ArrowUp'||event.key==='k'){event.preventDefault();if(items.length)selectFinding(items[Math.max(0,index<0?0:index-1)].id,true)}else if(event.key==='ArrowLeft'||event.key==='ArrowRight'){event.preventDefault();var modes=Array.from(sortSelect.options,function(option){return option.value});var position=modes.indexOf(state.sort)+(event.key==='ArrowRight'?1:-1);position=(position+modes.length)%modes.length;state.sort=modes[position];sortSelect.value=state.sort;render()}});
if(report.snapshot){verificationSelect.hidden=false;verificationSelect.value=state.verification;toolbar.classList.add('verification-enabled');statusSelect.querySelector('[value=resolved]').remove();sortSelect.querySelector('[value=age]').textContent='Line age (oldest first)'}
